...
grafico3 <- grafico2+ggtitle('Gráfico de dispersión temperatura máxima(MaxTemp) frente a temperatura mínima(MinTemp)')
grafico3

ggsave(filename = "./figure/Ejercicio1.pdf", device = "pdf")
```
...
#Se observa mejor con la representación del color.
grafico1 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, colour=RainToday))+geom_point()
grafico1
grafico2 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, shape=RainToday))+geom_point()
grafico2

grafico3 <- grafico1 + ggtitle('Gráfico que representa la presión frente a la temperatura a la misma hora del día, 9 de la mañana')
grafico3

ggsave(filename = "./figure/Ejercicio2.pdf", device = "pdf")
```
...
#En columnas se pueden comparar mejor los valores de las dos variables.
iris.grafico2+facet_grid(.~Measure)

iris.grafico3 <- ggplot(iris.tidy, aes(x = Species, y = Value, colour=Part))+geom_jitter()
iris.grafico3

ggsave(filename = "./figure/Ejercicio3.pdf", device = "pdf")
```
...
#Se puede observar la evolución pero no se ve muy claro, faltan líneas que unan los puntos.
p<-ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, colour=Fabricante))+geom_point()
p

p + geom_line()

grafico_final<-p + geom_line(aes(linetype=Fabricante))

grafico_final+ggtitle('Ventas mensuales de diferentes compañías tecnológicas')

ggsave(filename = "./figure/Ejercicio4.pdf", device = "pdf")
```
...
q <- ggplot(Experimento, aes(x=Dosis, y=Altura, linetype=Complemento, shape=Complemento))+geom_line()+geom_point()

r <- q+geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position=position_dodge(0.05))
r

s <- ggplot(Experimento, aes(x=Dosis, y=Altura, colour=Complemento))+       geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position=position_dodge(0.05))+                                     geom_line(aes(linetype=Complemento))+                                                     geom_point(aes(shape=Complemento))
s + theme_classic() + scale_color_brewer(palette="Reds")

ggsave(filename = "./figure/Ejercicio5.pdf", device = "pdf")
```
...
a <- ggplot(ds, aes(x=Year, y=MinTemp))+geom_boxplot()
a

b <- ggplot(ds, aes(x=Year, y=MaxTemp))                           +geom_violin(aes(x=Year, y=MaxTemp, colour=Year))+                         labs(title="Dristibución de temperaturas máximas por año",x="Año", y = "Temperatura(ºC)")
b

c <- ggplot(ds, aes(x=Year, y=MaxTemp))+                                     geom_violin(aes(x=Year, y=MaxTemp, colour=Year))+                                labs(title="Dristibución de temperaturas máximas por año",x="Año", y = "Temperatura(ºC)")+                                                    theme(legend.position = "none", axis.text.x = element_text(angle = 45))
c

ggsave(filename = "./figure/Ejercicio6.pdf", device = "pdf")
```
...
h <- ggplot(data=chol, aes(x=HEIGHT)) + geom_histogram()
h

i <- h+labs(title="Histograma de la variable Altura", x="Altura(cm)", y="Frecuencia")
i

j <- ggplot(data=chol, aes(x=HEIGHT)) + geom_histogram(breaks=seq(155,195,by=2))+  labs(title="Histograma de la variable Altura", x="Altura(cm)", y="Frecuencia")
j

k <- ggplot(data=chol, aes(x=HEIGHT)) + geom_histogram(breaks=seq(155,195,by=2), aes(y=stat(density)), col='red', fill='blue', alpha=.5)+                               labs(title="Histograma de la variable Altura", x="Altura(cm)", y="Frecuencia")+         geom_density(col="green") 
k

l <- ggplot(data=chol, aes(x=HEIGHT)) + geom_histogram(breaks=seq(155,195,by=2), aes(y=stat(density)), col='red', fill='blue', alpha=.5)+                               labs(title="Histograma de la variable Altura", x="Altura(cm)", y="Frecuencia")+         geom_density(col="green")+ facet_grid(.~SMOKE) 
l

ggsave(filename = "./figure/Ejercicio7.pdf", device = "pdf")
```
